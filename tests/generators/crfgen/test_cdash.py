...
from clinical_data_study_buddy.generators.crfgen.cdash import get_cdashig_variables_from_api, build_domain_crf
...
def test_get_cdashig_variables_from_api_with_pagination(mock_cdisc_library_client, monkeypatch):
    """
    ...
    mock_domains_response.field_links.domains = [MagicMock(href="/mdr/cdashig/v2.3/domains/AE")]

    # Mock the fields response for two pages
    mock_fields_response_page1 = MagicMock()
    mock_fields_response_page1.field_links.fields = [MagicMock(href=f"/mdr/cdashig/v2.3/domains/AE/fields/field_{i}") for i in range(10)]

    mock_fields_response_page2 = MagicMock()
    mock_fields_response_page2.field_links.fields = [MagicMock(href=f"/mdr/cdashig/v2.3/domains/AE/fields/field_{i}") for i in range(10, 15)]
    ...
    with patch("cdisc_library_client.api.cdash_implementation_guide_cdashig.get_mdr_cdashig_version_domains.sync") as mock_get_domains, \
         patch("cdisc_library_client.api.cdash_implementation_guide_cdashig.get_mdr_cdashig_version_domains_domain_fields.sync") as mock_get_fields, \
         patch("cdisc_library_client.api.cdash_implementation_guide_cdashig.get_mdr_cdashig_version_domains_domain_fields_field.sync") as mock_get_field_details_sync:

        mock_get_domains.return_value = mock_domains_response
        mock_get_fields.side_effect = [mock_fields_response_page1, mock_fields_response_page2, MagicMock(field_links=MagicMock(fields=[]))]
