...
class TestOpenFDAClient(unittest.TestCase):
    ...
    def test_get_adverse_events_with_date_filter(self, mock_get):
        ...
        client.get_adverse_events(
            "Aspirin", max_results=10, start_date="20200101", end_date="20210101"
        )
    ...
    def test_get_drug_label(self, mock_get):
        mock_response = MagicMock()
        mock_response.json.return_value = {
            "results": [{"id": "123", "openfda": {"brand_name": ["Aspirin"]}}]
        }
        mock_get.return_value = mock_response
...
class TestPopulators(unittest.TestCase):
    ...
    @patch("clinical_data_study_buddy.api.openfda.client.get_adverse_events")
    ...
    @patch("clinical_data_study_buddy.api.openfda.client.get_drug_label")
