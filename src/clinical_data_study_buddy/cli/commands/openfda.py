...
def openfda_populate_crf(
    drug_name: str = typer.Option(
        ..., "--drug-name", help="The name of the drug to search for."
    ),
    domain: str = typer.Option(
        ..., "--domain", help="The domain to populate (AE or LABEL)."
    ),
    max_results: int = typer.Option(
        10,
        "--max-results",
        help="The maximum number of records to return (for AE domain).",
    ),
    ...
    output_format: str = typer.Option(
        "csv", "--output-format", help="The output format (csv or json)."
    ),
):
    """
