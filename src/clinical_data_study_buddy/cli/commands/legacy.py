...
from clinical_data_study_buddy.generators.crfgen.exporter.registry import (
    get as get_exporter,
)
...
from clinical_data_study_buddy.generators.tfl.migration import (
    TFLSpecMigrationError,
    migrate_spec,
)
...
def generate(
    api_key: str = typer.Option(
        ..., envvar="CDISC_API_KEY", help="CDISC Library API key"
    ),
    ig_filter: Optional[str] = typer.Option(
        None, "--ig-filter", help="Filter IG by name"
    ),
    cache_path: Optional[pathlib.Path] = typer.Option(
        None, "-c", "--cache", help="Cache path"
    ),
    style_path: Optional[pathlib.Path] = typer.Option(
        None, "-s", "--style", help="Style path"
    ),
    template_path: Optional[pathlib.Path] = typer.Option(
        None, "-t", "--template", help="Template path"
    ),
    log_path: Optional[pathlib.Path] = typer.Option(
        None, "-l", "--log", help="Log file path"
    ),
    ...
    exporter = get_exporter(
        str(output_path),
        str(style_path) if style_path else None,
...
def protocol(
    therapeutic_area: str = typer.Option(
        ..., "--therapeutic-area", help="Therapeutic area of the study."
    ),
    treatment_arm: List[str] = typer.Option(
        ...,
        "--treatment-arm",
        help="A treatment arm of the study. Can be specified multiple times.",
    ),
    duration_weeks: int = typer.Option(
        ..., "--duration-weeks", help="Duration of the study in weeks."
    ),
    phase: int = typer.Option(..., "--phase", help="Phase of the study."),
    output_dir: str = typer.Option(
        "output", "--output-dir", help="Output directory for the protocol documents."
    ),
):
    """
...
@legacy_app.command()
def spec(
    spec_path: pathlib.Path = typer.Argument(
        ..., help="Path to the TFL specification file to validate."
    )
):
    """
    ...
    try:
        ...
        console.log("[red]Spec file is invalid:[/red]")
        console.log(e)
        raise typer.Abort()
