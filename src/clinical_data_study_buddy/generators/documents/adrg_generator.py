...
from clinical_data_study_buddy.generators.documents.base_document_generator import (
...
class ADRGGenerator(BaseDocumentGenerator):
    ...
    def _add_sections(self, document):
        ...
        document.add_paragraph(
            f"This document describes the analysis datasets for {self.study_config.study_id}."
        )
        document.add_paragraph(
            "This guide is intended to assist the reviewer in understanding the structure and content of the analysis datasets."
        )
        ...
        document.add_paragraph(
            f"Protocol Title: {self.study_config.protocol_title or 'N/A'}"
        )

        # Section 3: Analysis Datasets
        document.add_heading("3. Analysis Datasets", level=1)
        document.add_paragraph(
            "The following analysis datasets are included in this submission:"
        )
        ...
        document.add_paragraph(
            "The analysis datasets were checked for conformance with the CDISC ADaM standard."
        )
        document.add_paragraph(
            "Any conformance issues are documented in the data definition file (define.xml)."
        )

        # Section 5: Program and Macro Catalog
        document.add_heading("5. Program and Macro Catalog", level=1)
        document.add_paragraph(
            "This section provides a catalog of the programs and macros used to create the analysis datasets."
        )
        ...
        document.add_paragraph(
            "This section provides information about the data listings included in this submission."
        )

        # Section 7: Figures and Tables
        document.add_heading("7. Figures and Tables", level=1)
        document.add_paragraph(
            "This section provides information about the figures and tables included in this submission."
        )
