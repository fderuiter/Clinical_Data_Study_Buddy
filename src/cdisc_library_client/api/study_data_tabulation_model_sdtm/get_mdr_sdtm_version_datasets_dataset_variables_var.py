...
def _get_kwargs(
    ...
    _kwargs: dict[str, Any] = {
        "method": "get",
        "url": "/mdr/sdtm/{version}/datasets/{dataset}/variables/{var}".format(
            version=version,
            dataset=dataset,
            var=var,
        ),
...
) -> Optional[Union[DefaultErrorResponse, SdtmDatasetVariable]]:
    if response.status_code == 200:
        response_200 = SdtmDatasetVariable.from_dict(response.json())
...
def _build_response(
    *, client: Union[AuthenticatedClient, Client], response: httpx.Response
) -> Response[Union[DefaultErrorResponse, SdtmDatasetVariable]]:
    return Response(
...
def sync(
    ...
    *,
    ...
    Returns:
        Union[DefaultErrorResponse, SdtmDatasetVariable]
    """
