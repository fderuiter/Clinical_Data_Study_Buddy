...
def _get_kwargs(
    ...
    _kwargs: dict[str, Any] = {
        "method": "get",
        "url": "/mdr/cdashig/{version}/scenarios/{domain}.{scenario}".format(
            version=version,
            domain=domain,
            scenario=scenario,
        ),
...
) -> Optional[Union[CdashigScenario, DefaultErrorResponse]]:
    if response.status_code == 200:
        response_200 = CdashigScenario.from_dict(response.json())
...
def _build_response(
    *, client: Union[AuthenticatedClient, Client], response: httpx.Response
) -> Response[Union[CdashigScenario, DefaultErrorResponse]]:
    return Response(
...
def sync(
    ...
    *,
    ...
    Returns:
        Union[CdashigScenario, DefaultErrorResponse]
    """
